#Create table of ama proportional sharing between locations 
  ...
  for (cycle in 1:nrow(districts)){
    ...
    for (i in 1:nrow(haps_village)){
      if (haps_village$location[i]==districts$locations[iteration]&& !is.na(haps_village$location[i])){
        if (haps_village$date[i]>2014.293 &&haps_village$date[i]<2014.488){
...
Heatmap(as.matrix(district_mat), col = colorRamp2(c(0, 8, 25),c("blue", "yellow", "red")),cluster_rows=FALSE,cluster_columns=FALSE, na_col="white")
