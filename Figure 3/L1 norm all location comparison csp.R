#Create table of csp L1 norm between locations 
  ...
  for (cycle in 1:nrow(districts)){
    ...
    for (rowNum in 1:nrow(haps_village)){
      if (haps_village$location[rowNum]==districts$locations[iteration]&& !is.na(haps_village$location[rowNum])){
        if (haps_village$date[rowNum]>2014.293 &&haps_village$date[rowNum]<2014.488){
...
write.csv(districts, 'L1_2014_district_comparison.csv')

districts=read.csv("L1_2013_district_comparison.csv")
districts=districts[,3:ncol(districts)]
diag = nondiag = double()
...
Heatmap(as.matrix(districts),col = colorRamp2(c(16, 9, 6),c("blue", "yellow", "red")), cluster_rows=FALSE,cluster_columns=FALSE, na_col="white")
