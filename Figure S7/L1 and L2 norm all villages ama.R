...
haps_village = read.csv('ama allhaps_demog.csv')
villages = read.csv('ama_villages.csv')

for (iteration in 1:nrow(villages)){
  village_seqs = data.frame(haplotype=character(), location=character(), village=character(),freq=double(), ID=character(), household=character(), month=character())
...
write.csv(villages, 'ama L1L2norm village.csv')
##-----------------------------------
