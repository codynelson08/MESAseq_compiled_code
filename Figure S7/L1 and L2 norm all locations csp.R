...
locations = read.csv('csp_locations.csv')

for (iteration in 11:nrow(locations)){
  location_seqs = data.frame(haplotype=character(), location=character(), village=character(),freq=double(), ID=character(), location=character(), month=character())
  ...
  for (rowNum in 1:nrow(haps_village)){
    if (haps_village[rowNum,"location"]==locations$locations[iteration]&&!is.na(haps_village[rowNum,"location"])){
        location_seqs <- rbind(location_seqs, haps_village[rowNum,])
    }
  }
...
write.csv(locations, 'csp L1L2norm location.csv')
##-----------------------------------
