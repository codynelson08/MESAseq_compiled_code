...
household_match = read.csv('csp matched households time_control.csv')
...
for (iteration in 1:nrow(households)){
  ...
  print(paste0("Household is ", households$households[iteration], " and match is ", household_match$matched_household[iteration]) )
  ...
  for (rowNum in 1:nrow(haps_village)){
    ...
    if (haps_village[rowNum,"household"]==household_match$matched_household[iteration] && haps_village[rowNum,"person"]=='case household member'){
...
write.csv(households, 'csp household L1L2norm casechild swap temporal_match.csv')
##-----------------------------------
