...
library(circlize)
#program to identify the maximum proportional share score for each household

csp_matrix = read.csv ("csp weighted share score case_child matrix edited.csv")
ama_matrix = read.csv ("ama weighted share score case_child matrix edited.csv")
...
write.csv(district_df, "weighted geog sum.csv")
write.csv(time_df, "weighted time sum.csv")
