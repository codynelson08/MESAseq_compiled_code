...
library(circlize)
#program to identify the maximum L1 for each household

csp_matrix = read.csv ("csp household L1norm casechild matrix edited.csv")
ama_matrix = read.csv ("ama household L1norm casechild matrix edited.csv")
...
for (i in 1:length(households)){ #fill table with ranks
  ranks = order(rank(combined_matrix[i,], ties.method = "random"))
...
write.csv(district_df, "L1 geog sum.csv")
write.csv(time_df, "L1 time sum.csv")
