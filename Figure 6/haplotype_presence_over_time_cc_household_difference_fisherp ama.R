#Fisher p analysis of haplotype difference in cc and household for ama
...
ama_merged = read.csv('ama_haps_demog_merged_nocontrols.csv')
ama_merged = ama_merged[,c(2:126,250)]
...
ama_merged = ama_merged[,c(1:120,126:127)]
colnames (ama_merged) <- c(as.integer(1:120),"person",'interview_month')

ama_case_child = data.frame()
ama_household = data.frame()

for (i in 1:nrow(ama_merged)){
  if(ama_merged$person[i]=="case household member"&&!is.na(ama_merged$person[i])){
    ama_household=rbind(ama_household, ama_merged[i,])
  }
  if(ama_merged$person[i]=="case child"&&!is.na(ama_merged$person[i])){
    ama_case_child=rbind(ama_case_child, ama_merged[i,])
  }
}

# create a data frame summarizing each haplotype and the months it is present
# trying gathering the code to long format
long_ama_merged_case_child = long_ama_merged_household = data.frame()
long_ama_merged_case_child = gather(ama_case_child, H, reads, -interview_month) 
long_ama_merged_household = gather(ama_household, H, reads, -interview_month) 

# rename columns in ama_merged long file
names(long_ama_merged_case_child)[names(long_ama_merged_case_child) == 'reads'] <- 'reads_present'
names(long_ama_merged_case_child)[names(long_ama_merged_case_child) == 'H'] <- 'haplotype'
names(long_ama_merged_household)[names(long_ama_merged_household) == 'reads'] <- 'reads_present'
names(long_ama_merged_household)[names(long_ama_merged_household) == 'H'] <- 'haplotype'
  
# remove all rows with reads_present equal to 0
long_ama_merged_case_child = long_ama_merged_case_child[-which(long_ama_merged_case_child$reads_present == 0),]
long_ama_merged_case_child = long_ama_merged_case_child[-which(long_ama_merged_case_child$haplotype == "person"),]
long_ama_merged_household = long_ama_merged_household[-which(long_ama_merged_household$reads_present == 0),]
long_ama_merged_household = long_ama_merged_household[-which(long_ama_merged_household$haplotype == "person"),]

# summarize the new data set by month
month_summary_case_child = long_ama_merged_case_child %>% 
  group_by(interview_month,haplotype) %>%
  summarize(case_child_n=n())
month_summary_household = long_ama_merged_household %>% 
  group_by(interview_month,haplotype) %>%
...
ama_month_plot = ggplot(month_summary_merge, aes(as.numeric(x=interview_month), y=FisherP, color = n_difference)) +
 #geom_line()+
...
ggsave(ama_month_plot, filename="relprev graph pvalue ci ama.png", device="png",
