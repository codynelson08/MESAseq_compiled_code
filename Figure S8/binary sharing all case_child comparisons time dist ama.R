#ama binary sharing between all case children
...
ama_data <- read.csv('ama_haps_demog_merged_nocontrols_censored_seq.csv', header = T)

# only keep the first 95 columns
ama_merged = ama_data[,c(2:186,248:249, 310)]

# create a new variable that is just the month
str(ama_merged$interview_date)
ama_merged$month = NA
tmp=dmy(ama_merged$interview_date)
ama_merged$month = paste0(month(tmp),"-",year(tmp))

haps_village <- data.frame (haplotype=character(), location=character(), village=character(),freq=double(), ID=character(), household=character(), month=character(), person_type=character()) 
for(i in 1:nrow(ama_merged)){
  for(j in 1:(ncol(ama_merged)-9)){
    if (ama_merged[i,j]!=0){
      temp <- data.frame (colnames(ama_merged[j]), ama_merged[i,"location"], ama_merged[i,"village"], ama_merged[i,j], ama_merged[i,'labid'], ama_merged[i,'studyid_case_control_new'], ama_merged[i,'month'], ama_merged[i,'person_type'])
      haps_village <- rbind(haps_village,temp)
    }}}


colnames (haps_village) <- c('haplotype', 'location','village', 'freq', 'ID', 'household', 'month', 'person')
write.csv(haps_village, "ama_allhaps_demog_person.csv")
...
for (cc_iteration in 1:nrow(case_children_edited)){
  ...
  for (comp_cc_iteration in 1:nrow(case_children_edited)){
    ...
    cc_comps$share_score[line] = shared
  
  }
}


write.csv(cc_comps, 'ama share score all288 case_child comp time dist.csv')
