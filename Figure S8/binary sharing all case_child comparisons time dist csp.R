#csp binary sharing between all case children
...
for (cc_iteration in 1:nrow(case_children_edited)){
  ...
  for (comp_cc_iteration in 1:nrow(case_children_edited)){
    ...
    cc_comps$case_child[line] = as.character(case_children_edited$case_ID[cc_iteration])
...
write.csv(cc_comps, 'csp share score all283 case_child comp time dist.csv')
